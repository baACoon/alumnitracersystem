...
export const PendingSurvey = () => {
  ...
  useEffect(() => {
    ...
    const fetchTracer2Eligibility = async () => {
      try {
        const token = localStorage.getItem("token")
        // Get graduation year
        const gradRes = await axios.get(`http://localhost:5050/pending/gradyear/${userId}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        const gradYear = gradRes.data.gradyear

        // Get all Tracer2 surveys
        const res = await axios.get(`http://localhost:5050/surveys/tracer2/all/${userId}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
  ...
  const openSurveyModal = async (survey) => {
    try {
      const token = localStorage.getItem("token")
      const response = await axios.get(`http://localhost:5050/api/newSurveys/${survey._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const handleSubmitSurvey = async () => {
    try {
      ...
      await axios.post(`http://localhost:5050/api/newSurveys/${selectedSurvey._id}/response`, payload, {
        headers: { Authorization: `Bearer ${token}` },
      })
