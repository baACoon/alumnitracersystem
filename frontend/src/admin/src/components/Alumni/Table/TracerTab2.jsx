...
const Tracer2Tab = ({ studentData }) => {
  ...
  const tracer1Date = tracer1Survey?.createdAt ? new Date(tracer1Survey.createdAt) : null;
  const eligibilityDate = tracer1Date ? addYearsToDate(tracer1Date, 2) : null;
  const currentDate = new Date();
  const isEligible = eligibilityDate && currentDate >= eligibilityDate;

 // Case 1: Not eligible yet (plain text only)
 if (!tracer2Survey && !isEligible) {
  const unlockYear = eligibilityDate?.getFullYear() || 'N/A';

  return (
    <div className={`${styles.tracerContainer} ${styles.centerNotice}`}>
      <div className={styles.plainTextNotice}>
        <p>
          No Tracer 2 available. Come back in <strong>{unlockYear}</strong> after answering Tracer 1.
        </p>
      </div>
    </div>
  );
}
  // Case 2: Eligible but not yet submitted (show notification)
  if (!tracer2Survey && isEligible) {
    return (
