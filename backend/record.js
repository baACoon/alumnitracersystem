...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            ...
            return res
                .status(500)
                .json({error: 'Error generating unique user ID. Please try again.'});
        }
        ...
        if (result.insertedId) {
            ...
            res
                .status(201)
                .json({message: 'User registered successfully!', generatedID});
        } else {
            console.log('Error: Failed to insert user data');
            res
                .status(500)
                .json({error: 'Error registering user, insertion failed.'});
        }
    } catch (error) {
...
router.post('/login', async (req, res) => {
    ...
    try {
        if (!alumniID || !password) {
            ...
            return res
                .status(400)
                .json({error: 'Alumni ID and password are required.'});
        }

        const db = await connectToDatabase();
        const usersCollection = db.collection('students');

        // Log all users to help debug (remove in production)
        const allUsers = await usersCollection
            .find({})
            .toArray();
        console.log('All registered users:', allUsers.map(user => user.generatedID));
        ...
        if (!user) {
            ...
            return res
                .status(401)
                .json({error: 'Invalid Alumni ID or password.'});
        }
